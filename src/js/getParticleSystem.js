...
function getParticleSystem(params) {
  const { camera, emitter, parent, rate, texture, offsetY = 0 } = params;
  ...
  _points.frustumCulled = false; // right now we just disabled frustum culling, but in the future, maybe change the bounding box/sphere manually
  ...
  colorSpline.addPoint(1.0, new THREE.Color(0x808080)); // slightly darker
  // colorSpline.addPoint(1.0, new THREE.Color(0xff8080)); // this will add a slight red tint
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      _particles.push({
        position: new THREE.Vector3(
          ...
          (Math.random() * 2 - 1) * radius + offsetY,
